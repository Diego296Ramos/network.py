...
#Se crea la clase Network
    ...
    #Se hace el __init__ para inicializar la clase
        ...
        #Se crean las listas que almacenan las estimaciones de la inercia
        #para los sesgos y pesos de cada capa
        self.momentum_b = [np.zeros(b.shape) for b in self.biases]
        self.momentum_w = [np.zeros(w.shape) for w in self.weights]
        self.momentum_rate = 0.95 #Parametro para la tasa de inercia
    ...
    #Se crea la función u_m_b
        ...
        cost = self.cross_entropy_cost(self.feedforward(x), y) 
    ...
    #Función para actualizar los pesos y sesgos
        nabla_b = [np.zeros(b.shape) for b in self.biases]
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        #Listas que almacenan los gradientes acumulados sobre un minibatch
        for x, y in mini_batch: #Bucle for para cada ejemplo en el minibatch
            delta_nabla_b, delta_nabla_w = self.backprop(x, y)
            #Se almacenan los gradientes locales para cada ejemplo del
            #minibatch con el backprop
            nabla_b = [nb + dnb for nb, dnb in zip(nabla_b, delta_nabla_b)]
            #Se guardan los gradientes locales de los sesgos y pesos, sumandolos a los gradientes acumulados
            nabla_w = [nw + dnw for nw, dnw in zip(nabla_w, delta_nabla_w)]
        for i in range(len(self.biases)): # Ciclo for para las capas de la red
            self.momentum_b[i] = self.momentum_rate * self.momentum_b[i] - (eta / len(mini_batch)) * nabla_b[i]
            # Se calcula la actualización de la inercia para los sesgos de la i-esima capa
            # al usar el gradiente de la función de costo con respecto a los sesgos y la tasa de aprendizaje
            self.biases[i] += self.momentum_b[i]
            # Se actualizan los sesgos de la i-esima capa al sumarles la actualización, incorporando la inercia
        for i in range(len(self.weights)):
            self.momentum_w[i] = self.momentum_rate * self.momentum_w[i] - (eta / len(mini_batch)) * nabla_w[i]
            self.weights[i] += self.momentum_w[i]
            #Lo mismo para los pesos
    ...
    def cross_entropy_cost(self, output_activations, y):    
